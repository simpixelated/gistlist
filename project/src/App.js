...
- [x] Clicking on a gist should open up a detail page for that gist.
- [x] The detail of the gist should list all the files for that gist.
- [x] The contents of each file in the gist must be viewable somehow, but you can decide how best to do this.You can choose whether to show links to each file in the gist, a summary view of each file that expands, or the full content of each file on the detail page, etc.However, your app must render the file contents, so do not open external links to GitHub.Otherwise, there's no right or wrong choice here, just your personal preference.
...
- [x] Some basic navigation / header should make it possible to return from a particular gist to the list of gists for a user and to perform a search for a different user, without having to refresh the browser or use the back button.It is okay if the browser’s URL doesn’t change while navigating.
*/
